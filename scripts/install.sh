...
if ! echo "$PATH" | tr ':' '\n' | grep -qx "$cargo_bin"; then
  ...
  if [[ -n "$rc_file" ]] && grep -qF "$path_line" "$rc_file" 2>/dev/null; then
    ...
    if ask_yes_no "  Add it to ${rc_file}?"; then
      printf '\n# Added by yoink installer: put ~/.cargo/bin on PATH\n%s\n' "$path_line" >> "$rc_file"
...
# can see which version wrote it. Read from Cargo.toml (the build source of
# truth); fall back to the installed binary, then "unknown".
YOINK_VERSION="$(sed -n 's/^version = "\(.*\)"/\1/p' "$workspace_root/Cargo.toml" | head -n1)"
[[ -n "$YOINK_VERSION" ]] || YOINK_VERSION="$(command yoink --version 2>/dev/null | awk '{print $NF}')"
